...
public class LocalBinaryContentStorage {
    ...
    public ResponseEntity<Resource> download(Long id, Employee employee) {
        ...
        try {
            resource = new InputStreamResource(Files.newInputStream(resolvePath(id)));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        HttpHeaders headers = new HttpHeaders();
        headers.add(CONTENT_DISPOSITION, "attachment; filename=[" + employee.getDepartment().getName() + "]" + "_" + employee.getId());
